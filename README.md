#  [<img src="https://img.icons8.com/?size=100&id=H1TKqeNpWLQj&format=png&color=000000" alt="GitHub" width="30"/>](https://github.com) Lucas Rocha

💻 **Desenvolvedor Backend Java** | APIs REST | Spring Boot | Docker | PostgreSQL  

Desenvolvedor Backend com foco em Java e Spring Boot, especializado na construção de APIs REST seguras e escaláveis. Tenho experiência prática no desenvolvimento de sistemas com autenticação JWT, integração com banco de dados PostgreSQL e deploy em ambiente Docker. Recentemente desenvolvi um sistema completo de gerenciamento de usuários, incluindo autenticação, autorização e persistência de dados, aplicando boas práticas de arquitetura e segurança.
---

## 🚀 Sobre mim

- 👨‍💻 Desenvolvedor Backend com foco em **APIs escaláveis e arquitetura limpa**  
- ⚙️ Experiência sólida em **Spring Boot, JWT, Docker, PostgreSQL e MySQL**  
- 🌱 Aprendizado contínuo e aplicação de boas práticas de desenvolvimento  
- 🔍 Entrega de **código limpo, seguro e eficiente**

---

## 🛠 Habilidades Técnicas

| Linguagens | Frameworks | DevOps | Ferramentas |
|------------|------------|--------|-------------|
| Java | Spring Boot | Docker | Git / GitHub |
| SQL / PostgreSQL / MySQL | Hibernate | CI/CD Básico | Maven / Postman |

### Níveis de experiência

| Habilidade | Nível |
|------------|-------|
| Java | 🔹🔹🔹🔹🔹 (Avançado) |
| Spring Boot | 🔹🔹🔹🔹 (Avançado) |
| PostgreSQL / MySQL | 🔹🔹🔹🔹 (Avançado) |
| Docker | 🔹🔹🔹 (Intermediário) |
| JWT / Segurança | 🔹🔹🔹🔹 (Intermediário) |
| Git / GitHub | 🔹🔹🔹🔹 (Avançado) |
| Postman / API Testing | 🔹🔹🔹🔹 (Avançado) |

---

## 🧠 Soft Skills

- Comunicação clara e objetiva  
- Trabalho em equipe e colaboração remota  
- Resolução de problemas complexos  
- Organização e gerenciamento de tarefas  
- Aprendizado contínuo  

---

## 🗺 Roadmap de Aprendizado

### Habilidades Atuais
[![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=java&logoColor=white)](https://www.java.com/)  
[![Spring Boot](https://img.shields.io/badge/Spring%20Boot-6DB33F?style=for-the-badge&logo=spring&logoColor=white)](https://spring.io/projects/spring-boot)  
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)](https://www.postgresql.org/)  
[![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)](https://www.docker.com/)

### Skills Futuras
[![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=for-the-badge&logo=kubernetes&logoColor=white)](https://kubernetes.io/)  
[![Microservices](https://img.shields.io/badge/Microservices-F05032?style=for-the-badge&logo=redhat&logoColor=white)](https://microservices.io/)  
[![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white)](https://aws.amazon.com/)  
[![Kafka](https://img.shields.io/badge/Kafka-231F20?style=for-the-badge&logo=apachekafka&logoColor=white)](https://kafka.apache.org/)

---

## 🏅 Conquistas e Certificações

- Certificação Java SE 11 Developer – Oracle - Em Progresso. 




---

## 📊 Estatísticas GitHub

 

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=lucasrochadnsx&theme=radical)  

---

## 📫 Contato

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/lrdns)  
📧 lucas.rochadnsx@gmail.com  

✨ Obrigado por visitar meu GitHub!
